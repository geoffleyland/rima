...
function iterator:__eval(S, eval)
  ...
  return iterator:new(self.base_, exp, self.key_, self.value_, self.set_)
end
...
-- Iterators don't have an __index, because indexing is more complex than
-- indexing the iterator's value - there might be default values to deal with.
