...
function tabulate_type:__repr(format)
  return ("tabulate(%s, %s)"):format(
    lib.repr(self.indexes, format),
    lib.repr(self.expression, format))
end
...
function tabulate_type:__address(S, a, i, eval)
  if #a - i + 1 < #self.indexes then
    error(("tabulate: error evaluating '%s' as '%s': the tabulation needs %d indexes, got %d"):
      format(__repr(self), lib.repr(self.expression), #self.indexes, #a - i + 1), 0)
  end
  local S2
  if S then
  ...
  if not status then
    ...
    local args = lib.concat(self.indexes, ", ",
      function(si) i = i + 1; return ("%s=%s"):format(si.names[1], lib.repr(a:value(i))) end)
    error(("tabulate: error evaluating '%s' as '%s' where %s:\n  %s"):
      format(__repr(self), lib.repr(self.expression), args, r:gsub("\n", "\n  ")), 0)
  end
