...
--  T:expect_ok(function() mul:check({{1, 2}}) end) 
  ...
  T:check_equal(mul.__dump({{1, 1}}), "*(number(1)^1)")
  T:check_equal(mul.__rima_tostring({{1, 1}}), "1")
  T:check_equal(mul.__dump({{1, 2}, {3, 4}}), "*(number(2)^1, number(4)^3)")
  T:check_equal(mul.__rima_tostring({{1, 2}, {3, 4}}), "2*4^3")
  T:check_equal(mul.__rima_tostring({{-1, 2}, {3, 4}}), "1/2*4^3")
  T:check_equal(mul.__rima_tostring({{-1, 2}, {-3, 4}}), "1/2/4^3")

  local S = rima.scope.new()
  T:check_equal(mul.__eval({{1, 2}}, S), 2)
  T:check_equal(mul.__eval({{1, 2}, {3, 4}}, S), 128)
  T:check_equal(mul.__eval({{2, 2}, {1, 4}, {1, 6}}, S), 96)
  T:check_equal(mul.__eval({{2, 2}, {1, 4}, {-1, 6}}, S), 8/3)
  T:check_equal(mul.__eval({{2, 2}, {1, 4}, {1, -6}}, S), -96)

  local a, b = rima.R"a,b"
  rima.scope.set(S, {a = 5, b = rima.positive()})
  T:check_equal(mul.__dump({{1, a}}), "*(ref(a)^1)")
  T:check_equal(mul.__eval({{1, a}}, S), 5)
  T:check_equal(mul.__eval({{1, a}, {2, a}}, S), 125)
  ...
  T:check_equal(expression.dump(mul.__eval({{2, b}}, S)), "*(ref(b)^2)")
  T:check_equal(expression.dump(mul.__eval({{1, b}}, S)), "ref(b)", "checking we simplify identity")
  ...
  T:check_equal(mul.__eval({{0, S.b}}, S), 1, "checking we simplify 0")
