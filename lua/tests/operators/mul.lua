...
--  T:expect_ok(function() mul:check({{1, 2}}) end) 
  ...
  T:check_equal(expression.eval(b / b, S), 1)
  T:check_equal(expression.eval(b * b, S), "b^2")
  T:check_equal(expression.eval(2 * (3 * b), S), "6*b")
  T:check_equal(expression.eval(2 / (3 * b), S), "0.6667/b")

  T:check_equal(expression.eval(2 * (3 * a), S), 30)
  T:check_equal(expression.eval(2 / (3 * a), S), 2/15)
  ...
  T:check_equal(expression.dump(expression.eval(1 * b, S)), "ref(b)", "checking we simplify identity")
  T:check_equal(expression.dump(expression.eval(2 * b / 2, S)), "ref(b)", "checking we simplify identity")

  T:check_equal(mul:eval(S, {{0, S.b}}), 1, "checking we simplify 0")
  T:check_equal(expression.eval(0 * S.b, S), 0, "checking we simplify 0")
