...
--  T:expect_ok(function() add:check({{1, 2}}) end) 
  ...
  T:check_equal(add.__dump({{1, 1}}), "+(1*number(1))")
  T:check_equal(add.__rima_tostring({{1, 1}}), "1")
  T:check_equal(add.__dump({{1, 2}, {3, 4}}), "+(1*number(2), 3*number(4))")
  T:check_equal(add.__rima_tostring({{1, 2}, {3, 4}}), "2 + 12")
  T:check_equal(add.__rima_tostring({{-1, 2}, {3, 4}}), "-2 + 12")
  T:check_equal(add.__rima_tostring({{-1, 2}, {-3, 4}}), "-2 - 12")

  local S = rima.scope.new()
  T:check_equal(add.__eval({{1, 2}}, S), 2)
  T:check_equal(add.__eval({{1, 2}, {3, 4}}, S), 14)
  T:check_equal(add.__eval({{1, 2}, {3, 4}, {5, 6}}, S), 44)
  T:check_equal(add.__eval({{1, 2}, {3, 4}, {-5, 6}}, S), -16)
  T:check_equal(add.__eval({{1, 2}, {3, 4}, {5, -6}}, S), -16)

  local a, b, c = rima.R"a,b,c"
  rima.scope.set(S, { a = 5, b = rima.positive() })
  T:check_equal(add.__dump({{1, a}}), "+(1*ref(a))")
  T:check_equal(add.__eval({{1, a}}, S), 5)
  T:check_equal(add.__eval({{1, a}, {2, a}}, S), 15)
  ...
  T:check_equal(expression.dump(add.__eval({{2, S.b}}, S)), "+(2*ref(b))")
  T:check_equal(expression.dump(add.__eval({{1, S.b}}, S)), "ref(b)", "checking we simplify identity")
